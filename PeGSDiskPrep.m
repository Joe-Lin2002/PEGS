...
for frame = 1:nFrames %Loops for total number of images
    ...
    % Additional check for blue and red particles
    for n = 1:N
        for pointer = 1:N
            if n ~= pointer % Ensure not to compare the particle with itself
                ...
                % If the distance between centers is less than the radius of the larger circle
                if distance < 0.5 * particle(pointer).r
                    % Condition 1: Blue circle inside a red circle
                    if particle(n).color == 'b' && particle(pointer).color == 'r'
                        toDelete(n) = true;
                        break; % Break the inner loop as we only need to mark the particle once
                    end

                    % Condition 2: Two circles have same color, delete the smaller one
                    if particle(n).color == particle(pointer).color
                        if particle(n).r < particle(pointer).r
                            toDelete(n) = true;
                        else
                            toDelete(pointer) = true;
                        end
                        break; % Break the inner loop as we only need to mark the particle once
                    end
                end
